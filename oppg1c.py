...
def r2(z, z_tilde):
    ...
    return 1 - np.sum((z - z_tilde)**2)/np.sum((z - z_mean)**2)
...
def MSE_r2_beta_plot(order, M, r, b, n):
    ...
    for i in tqdm(range(order)):
        ...
        for j in range(n):
            seed = seeds[j]
            # MSE_train, R2_train, z_tilde_train, MSE_test, R2_test, z_tilde_test, beta, \
            #     MSE_train_scaled, R2_train_scaled, z_tilde_train_scaled, MSE_test_scaled, \
            #         R2_test_scaled, z_tilde_test_scaled, beta_scaled = OLS(i+1, False, False, seeds[j])

            MSE_train_scaled, MSE_test_scaled, R2_train_scaled, R2_test_scaled, beta_scaled = OLS(x, y, z, order, seed, scale=True)
            MSE_train, MSE_test, R2_train, R2_test, beta = OLS(x, y, z, order, seed, scale=True)
        ...
        # MSE_train, R2_train, z_tilde_train, MSE_test, R2_test, z_tilde_test, beta, \
        #     MSE_train_scaled, R2_train_scaled, z_tilde_train_scaled, MSE_test_scaled, \
        #         R2_test_scaled, z_tilde_test_scaled, beta_scaled = OLS(i+1, False, False, seed)
...
def bootstrap(x, y, z, seed, order, n_boostraps):
    from sklearn.model_selection import train_test_split
...
if _name__=="__main_":
    ...
    bootstrap(x, y, z, seed, order, n_boostraps)
